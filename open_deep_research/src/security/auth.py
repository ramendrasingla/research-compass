...
async def get_current_user(authorization: str | None) -> Auth.types.MinimalUserDict:
    ...
    1. Local development: When SUPABASE_URL and SUPABASE_KEY are not set,
       it allows unauthenticated access with a default local user.
    2. Production (Open Agent Platform): When Supabase is configured,
       it validates JWT tokens against Supabase authentication.

    Args:
        authorization: The Authorization header value (format: "Bearer <token>")

    Returns:
        User dictionary with identity

    Raises:
        HTTPException: For authentication errors in production mode
    """

    # Local development mode: No Supabase configured
    if not supabase:
        # Allow unauthenticated access for local development
        # This enables running LangGraph Studio locally without authentication setup
        return {
            "identity": "local-dev-user",
        }

    # Production mode: Supabase authentication required
    ...
    # Parse the authorization header (format: "Bearer <token>")
    try:
